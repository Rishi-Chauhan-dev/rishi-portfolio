<!--
  GITHUB PROFILE README
  ─────────────────────
  This becomes your GitHub landing page. To use it:
  1. Create a new repo named EXACTLY your username (e.g. "rishichauhan").
  2. Make it public and add a README.
  3. Paste this file's contents as that README.md.

  Then replace every "rishichauhan" below with your real GitHub username,
  and update the LinkedIn / portfolio / email links.
-->

<div align="center">

# Hi, I'm Rishi Chauhan 👋

### UI/UX Developer — I design in Figma and build in React / Next.js

*I care about the small stuff between design and code: the spacing, the easing curve,*
*and the moment an interface stops feeling like software and starts feeling effortless.*

[![Portfolio](https://img.shields.io/badge/Portfolio-16150f?style=for-the-badge&logo=vercel&logoColor=white)](https://rishichauhan.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/rishichauhan)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:rishi.chauhan24092002@gmail.com)

</div>

---

### 🎯 About me

- 🎓 Final-year **B.Tech CSE** student at **NSIT** (2022–2026)
- 🛍️ Shipped a **live e-commerce storefront** end-to-end → [shreenavyafoodproducts.com](https://www.shreenavyafoodproducts.com/)
- 🎨 I design **new-gen product interfaces** across fintech, wellness and AI
- 🌉 My thing: **design → code with nothing lost in the handoff**

### 🧰 Toolkit

**Design**
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![Prototyping](https://img.shields.io/badge/Prototyping-16150f?style=flat)
![Design Systems](https://img.shields.io/badge/Design_Systems-16150f?style=flat)
![Accessibility](https://img.shields.io/badge/Accessibility-16150f?style=flat)

**Build**
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-0055FF?style=flat&logo=framer&logoColor=white)

### 🚀 Featured work

| Project | What it is | |
| :-- | :-- | :-- |
| **Shree Navya** 🟢 Live | Indian FMCG e-commerce storefront | [Visit ↗](https://www.shreenavyafoodproducts.com/) |
| **Aurora** | Neobank / fintech mobile concept | [Case study ↗](https://rishichauhan.vercel.app/work/aurora) |
| **Bloom** | Wellness & fitness mobile app | [Case study ↗](https://rishichauhan.vercel.app/work/bloom) |
| **Lumen** | AI SaaS analytics dashboard | [Case study ↗](https://rishichauhan.vercel.app/work/lumen) |

### 📊 GitHub

<div align="center">

![Rishi's GitHub stats](https://github-readme-stats.vercel.app/api?username=Rishi-Chauhan-dev&show_icons=true&hide_border=true&title_color=4d6a00&icon_color=4d6a00)
![Top languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Rishi-Chauhan-dev&layout=compact&hide_border=true&title_color=4d6a00)

</div>

<div align="center">

**Currently open to UI/UX roles & freelance.** Let's build something worth shipping.

</div>
